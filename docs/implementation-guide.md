...
| `GitHub API error 403` after updating PAT | Lambda has cached the old token (5-min TTL) | Update a Lambda env var to force a cold start, then retry |
| Argo CD app shows `ComparisonError` | kustomize build fails | Run `kustomize build gitops/clusters/staging` locally to see the error |
